...
namespace BIOMETRICS_JULY
{
    public partial class ClockIn : Form
    {
        ...
        void ValidateUser()
        {
            ...
            //  string constring = @"Server=localhost; Database=biometrics; UID =root; PWD=1234;";
            using (MySqlConnection con = new MySqlConnection(Helper.GetConnection()))
            {
                ...
                // query = @"SELECT userID, Thumb FROM users";
                using (MySqlCommand cmd = new MySqlCommand(query, con))
                {
                    using (MySqlDataReader data = cmd.ExecuteReader())
                    {
                        ...
                        while (data.Read())
                        {
                            ...
                            clockInUserControl1.Samples.Add(new DPFP.Template(new MemoryStream(fingerPrint)), employeeid);
                        }
